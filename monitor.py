...
logger = logging.getLogger('systemsync')
...
def get_watch_dirs(config, user_name):
    watch_dirs = []
    for key, value in config.items('systemsync.dirs'):
...
def get_clients(config):
    clients = []
    for key, value in config.items('systemsync.clients'):
        words = value.split(',')
...
def get_server_tuple(config):
    server_uname, server_ip, server_port = config.get('systemsync.server', 'server', 1).split(',')
...
def main():
    ...
    #start logging
    setup_logging("systemsync.log.%s-%s" % (args.ip, args.port));
    logger = logging.getLogger('systemsync')

    #Read config file
    config = ConfigParser.ConfigParser()
    logger.info("Using config file: systemsync.cfg")
    config.read('systemsync.cfg')
